...
class Set(commands.Cog):
    ...
    async def set(self, ctx):
        embed = discord.Embed(
            title="set **<command>**", colour=discord.Colour(0xE9ACFD)
        )
        embed.set_footer(text=f"{ctx.guild}", icon_url=f"{ctx.guild.icon_url}")
        embed.add_field(
            name="roleid", value="Set message id for role reactions.", inline=False
        )
        embed.add_field(
            name="welctext",
        ...
        embed.set_footer(
            text=f"Help requested by: {ctx.author}", icon_url=ctx.author.avatar_url
        )
    ...
    async def set_msg_id_role(self, ctx, *, role_id):
        self.cursor.execute(
            f"SELECT msg_id_reaction FROM main WHERE guild_id = ('{str(ctx.guild.id)}')"
        )
    ...
    async def set_welc_text(self, ctx, *, welc_text):
        ...
        self.cursor.execute(
            f"SELECT welc_text FROM main WHERE guild_id  = ('{str(ctx.guild.id)}')"
        )
        result = self.cursor.fetchone()
    ...
    async def set_welc_ch(self, ctx, welc_ch: discord.TextChannel):
        ...
        self.cursor.execute(
            f"SELECT ch_id_welcome FROM main WHERE guild_id = ('{str(ctx.guild.id)}')"
        )
        result = self.cursor.fetchone()
