...
class Welcome(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        # self.cursor.execute(f'SELECT ch_id_welcome FROM main WHERE guild_id=%s', (str(member.guild.id)))
        # result = self.cursor.fetchone()
        # if result is None:
        #     return
        # else:
        #     self.cursor.execute(f'SELECT welc_text FROM main WHERE guild_id=%s', (str(member.guild.id)))
