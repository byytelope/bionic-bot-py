...
class Welcome(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        self.cursor.execute(
            f"SELECT ch_id_welcome FROM main WHERE guild_id = ('{str(member.guild.id)}')"
        )
        ...
        if result is None:
            ...
            self.cursor.execute(
                f"SELECT welc_text FROM main WHERE guild_id = ('{str(member.guild.id)}')"
            )
            result_1 = self.cursor.fetchone()
            welc_ch = self.bot.get_channel(id=int(result[0]))
            msg = str(result_1[0]).format(
                mention=member.mention, user=member.name, guild=member.guild
            )
